...
function mapRawExchangeHistoryResult(
  rawResult: RawTimeSeriesResult,
  toCurrency: string
): ExchangeRateHistoryItem[] {
...
export async function getTimeSeriesData(params: {
  ...
  const response = await fetch(
    `${baseExchangeUrl}/timeseries?start_date=${formatDate(
      from
    )}&end_date=${formatDate(to)}&base=${params.fromCurrency}`
  );
