...
const useViewModel = () => {
  ...
  const [selectedFilterVariant, setSelectedFilterVariant] = useState(
    variants[0].value
  );

  const historyRaws = useMemo(() => {
    return service.timeSeriesHistory
      .sort((a, b) => {
  ...
  return (
    <Grid container spacing={2}>
      <Grid item xs={12} marginTop={4}>
        <Typography variant={"h3"}>Exchange History</Typography>
      </Grid>
      <Grid item xs={2} marginTop={2}>
        <TextField
          ...
          onChange={(e) =>
            viewModel.setSelectedFilterVariant(Number(e.target.value))
          }
